...
TMP_DIR = Path(os.getenv("TMP_DIR", str(UPLOAD_DIR / "_tmp")))
TMP_DIR.mkdir(parents=True, exist_ok=True)

TARGET_PX = int(os.getenv("TARGET_PX", "3000"))
EDITOR_PX = int(os.getenv("EDITOR_PX", "1200"))  # smaller = faster UI

# SOFT limit = what you *prefer*; HARD limit = absolute max we’ll accept
MAX_UPLOAD_MB = int(os.getenv("MAX_UPLOAD_MB", "12"))          # soft
HARD_UPLOAD_MB = int(os.getenv("HARD_UPLOAD_MB", "35"))        # hard
MAX_UPLOAD_BYTES = MAX_UPLOAD_MB * 1024 * 1024
HARD_UPLOAD_BYTES = HARD_UPLOAD_MB * 1024 * 1024
...
# ----------------------------
# Helpers: file read to disk (stream + hard cap)
# ----------------------------
async def _stream_upload_to_tempfile(file: UploadFile, hard_limit_bytes: int) -> Tuple[Path, int]:
    """
    Streams upload to disk to avoid RAM spikes.
    Returns (tmp_path, total_bytes).
    Raises ValueError("too_large") if exceeds hard_limit_bytes.
    """
    tmp_path = TMP_DIR / f"up_{uuid.uuid4().hex}"
    total = 0
...
def _pil_open_path_safe(path: Path) -> Image.Image:
    """
    Opens from a file path (supports PNG/JPG/WebP if Pillow supports it),
    applies EXIF transpose, forces RGBA, enforces max pixels.
    """
    try:
...
def _bg_process_session(session_id: str):
    try:
        ...
        # Don’t brick the session — user can still edit original
        try:
            p = _paths(session_id)
            if p["orig"].exists():
...
async def upload_image(
    ...
    # Stream upload to disk (avoid RAM spikes); allow bigger than soft limit up to hard limit
    try:
        tmp_path, total = await _stream_upload_to_tempfile(file, HARD_UPLOAD_BYTES)
    except ValueError as e:
        if str(e) == "too_large":
            return JSONResponse(
                {"error": f"File too large (max {HARD_UPLOAD_MB}MB). Try a smaller export or screenshot."},
                status_code=413,
            )
    ...
    try:
        try:
            img = _pil_open_path_safe(tmp_path)
        except ValueError as e:
        ...
        # Always downscale for the editor UI (this is what makes giant phone photos safe)
        ...
        # If file was bigger than SOFT limit, note it (for /status debugging)
        oversized = total > MAX_UPLOAD_BYTES
...
async def save_edit(session_id: str, file: UploadFile = File(...)):
    ...
    # Stream to disk; accept up to HARD cap
    try:
        tmp_path, total = await _stream_upload_to_tempfile(file, HARD_UPLOAD_BYTES)
    except ValueError:
        return JSONResponse(
            {"error": f"File too large (max {HARD_UPLOAD_MB}MB)"},
            status_code=413,
        )

    try:
        try:
            browser_img = _pil_open_path_safe(tmp_path)
        except ValueError:
...
async def storefront_request(
    ...
    # If Shopify form did NOT use the editor session, allow direct file upload (and normalize)
    if not main_session_id:
        ...
        # Stream to temp
        try:
            tmp_main, total = await _stream_upload_to_tempfile(storefront_logo_file, HARD_UPLOAD_BYTES)
        except ValueError:
            return JSONResponse({"error": f"File too large (max {HARD_UPLOAD_MB}MB)"}, status_code=413)

        try:
            try:
                img_main = _pil_open_path_safe(tmp_main)
            except ValueError as e:
                if str(e) == "too_many_pixels":
                    return JSONResponse({"error": "Image resolution too large. Please upload a smaller image."}, status_code=413)
        ...
        if storefront_logo_secondary:
            try:
                tmp_sec, total2 = await _stream_upload_to_tempfile(storefront_logo_secondary, HARD_UPLOAD_BYTES)
                try:
                    img_sec = _pil_open_path_safe(tmp_sec)
